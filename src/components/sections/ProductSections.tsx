...
import { Bot, CalendarDays, Headphones, Workflow } from 'lucide-react';
...
export function ProductSections() {
  return (
    <>
        ...
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-3">
          ...
          <GlassCard className="lg:col-span-2">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-4">
              {automationOptions.map(({ icon: Icon, label }) => <div key={label} className="rounded-2xl border border-white/10 bg-black/20 p-4"><Icon className="h-5 w-5 text-vexel-blue" /><p className="mt-4 text-sm text-vexel-muted">{label}</p></div>)}
            </div>
          </GlassCard>
        </div>
      </section>
      <section id="sectores" className="relative px-5 py-28 sm:px-8 lg:px-10">
      ...
      <section id="pacotes" className="relative px-5 py-28 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-10 grid gap-4 lg:grid-cols-4">
            {packages.map((pack) => <GlassCard key={pack.name}><h3 className="text-2xl font-semibold text-vexel-ice">{pack.name}</h3><p className="mt-2 text-sm text-vexel-blue">{pack.ideal}</p><ul className="mt-6 space-y-3 text-sm text-vexel-muted">{pack.items.map((item) => <li key={item} className="flex gap-3"><span className="mt-2 h-1.5 w-1.5 rounded-full bg-vexel-blue" />{item}</li>)}</ul><p className="mt-7 rounded-full border border-white/10 px-4 py-2 text-center text-xs uppercase tracking-[0.18em] text-vexel-muted">Sob diagnóstico</p></GlassCard>)}
          </div>
        </div>
      </section>
    </>
